...
def db_insert(db_name, collection_name, data_obj):
  ...
  data_obj['_id'] = 1
  result = coll.replace_one({'_id': 1}, data_obj, upsert=True) 
  return result # return ObjectId of item inserted 

def call_baseline(query, dayQuery=True, matlab_engine=None):
  # extract paramater
  if dayQuery:
    param = query['day']
  else:
  ...
  if matlab_engine is None:
    ...
    matlab_engine.baseline_ka(param)
  ...
  #TODO: transform response data into name-value pairs
...
def call_evaluator(query, matlab_engine=None):
  #TODO: write input json file needed by matlab function
  input_file = './MATLAB/Penn-Analytics/input_evaluator.json'
  #TODO: validate set point values, determine which is being set
  input_data = {}
...
def call_searchbin(query, matlab_engine=None):
  ...
  if matlab_engine is None:
    ...
    matlab_engine.func_name(binNum)
...
def start_server(ipaddr, port):
  ...
  while True:
    ...
    try:
      ...
      #elif query['type'] == 2:
        #response = call_baseline(query, dayQuery=True, eng)
      #elif query['type'] == 3:
        #response = call_baseline(query, dayQuery=False, eng)
      elif query['type'] == 4:
